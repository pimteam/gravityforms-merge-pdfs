...
function gf_merge_pdfs_get_files( int $entry_id ) : iterable {
	...
	if(!empty($_GET['gf_merge_pdfs'])) {
		...
		if ( $include_form_pdf ) {
			error_log('gfpdf_init fired? ' . ( did_action('gfpdf_init') ? 'yes' : 'no' ));
 ...
 */
function gf_merge_pdfs_output( $files, $errors, $entry_id, $file_name = '', $return_content = false, $no_cache = false ) {
	...
	* ------------------------------------------------------------------- */
	...
	if ( file_exists( $cached_file ) and !$no_cache) {
